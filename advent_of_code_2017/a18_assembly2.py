...
class Processor(object):
    ...
    def run_instruction(self):
        ...
        try:
            ...
            x = inst[1]
            comp_val = self.registers.get(x,0)
        if len(inst) == 3:
            try:
                y = int(inst[2])
            except ValueError:
        ...
        # snd X sends value of X to the remote processor
        if cmd == 'snd':
            if type(x) is int:
                self.remote_processor.enqueue(x)
            else:
                self.remote_processor.enqueue(self.registers.get(x,0))
            self.values_sent += 1
        # set X Y sets register X to the value of Y.
        elif cmd == 'set':
        ...
        # mod X Y sets register X to the remainder of dividing the value contained in register X by the value of Y (that is, it sets X to the result of X modulo Y).
            ...
            self.registers[x] = self.registers.get(x,0) % y
        # rcv X receives the next queued value. If there is no value in the queue
        #  block and wait for a value to appear
        elif cmd == 'rcv':
            if not self.receive_queue:
                self.program_counter -= 1 # try again next round
                return False
            else:
