...
def connects_to(prog):
    connected = set()
    to_check = set([prog])
    while len(to_check) > 0:
        checking = to_check.pop()
        connected.add(checking)
        to_check.update(connections[checking].difference(connected))
    print(connected)
    return connected

f = open('a12_pipes_input.txt', 'r')
lines = 0
...
for line in f:
    line.strip()
    from_prog, to_progs = line.split('<->')
    from_prog = int(from_prog.strip())
    to_progs = {int(x.strip()) for x in to_progs.split(',')}
    connections[from_prog] = to_progs

remaining = {x for x in connections}
groups = 0
while remaining:
    groups += 1
    prog = remaining.pop()
    remaining = remaining.difference(connects_to(prog))
