...
weight_per_container = total_weight // 3
...
def can_balance(presents, this_way = None, wpc = None):
    """Can a set of presents be split into two equal sets?"""
    if this_way is None:
        this_way = set()
    if wpc is None:
        wpc = sum(presents) / 2
    ...
    for i in range(len(presents)):
        ...
        if can_balance(presents[i+1:], this_way, wpc):
            return True
...
def ways_to_make_weight(presents, wpc, ways, this_way = None):
    ...
    if sum(this_way) == wpc:
        ...
        if can_balance(list(set(all_presents).difference(this_way))):
