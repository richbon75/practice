    ...
    for armor in armors:
        ...
        for r in range(0, len(rings)):
            ...
            for r2 in range(r+1, len(rings)):
                ...
                if player.can_i_beat(monster):
                    if player.inventory_value < mincost:
                        mincost = player.inventory_value
                        print('Player wins with {} costing {}'.format(player.show_inventory(), player.inventory_value))
                player.unquip()
            player.unquip()
        player.unquip()
    player.unquip()
